...
During sampling, we hook a chosen FLUX transformer block (for example block 15) and capture the feedforward output tensor for a chosen diffusion step index \(t\). The captured activations are tokenized spatially and have feature dimension \(C=3072\) for the experiments in this repo.
